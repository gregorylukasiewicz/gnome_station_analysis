...
# complex lorentz function
def complex_lorentz_doubleside(f, f0, A,gamma, phi):
    """Summ of two complex lorentzians centered on +f0 and -f0
    ...
    .. math::
        Ae^{i\phi}\\frac{\gamma - i(f-f_0)}{\gamma^2 + (f-f_0)^2}+Ae^{i\phi}\\frac{\gamma - i(f+f_0)}{\gamma^2 + (f+f_0)^2}

    """
    return A*np.exp(1j*phi)*(gamma-1j*(f-f0))/(gamma**2+(f-f0)**2)+A*np.exp(1j*phi)*(gamma-1j*(f+f0))/(gamma**2+(f+f0)**2)
...
# vector model for complex_lorentz
def vec_model_doubleside(freq, f0, A, gamma, phi):
    """Vector model for complex lorentzian function (complex_lorentz)
    ...
    """
    real_part = np.real(complex_lorentz_doubleside(freq, f0, A,gamma, phi))
    imag_part = np.imag(complex_lorentz_doubleside(freq, f0, A,gamma, phi))
    return np.hstack([real_part,imag_part])
...
# complex_lorentz + complex linear background (4 additional params)
def complex_lorentz_doubleside_lin_back(f, f0, A, gamma, phi, a_real, b_real, a_imag, b_imag):
    """Complex lorentzian function with complex linear background
    ...
    """
    lor = complex_lorentz_doubleside(f, f0, A, gamma, phi)
    back = (a_real + 1j * a_imag) * f + (b_real + 1j * b_imag)
    return lor + back
...
def vec_model_lin_back(freq, f0, A, gamma, phi, a_real, b_real, a_imag, b_imag):
    ...
    """
    real_part = np.real(complex_lorentz_doubleside(freq, f0, A, gamma, phi)) + a_real * freq + b_real
    imag_part = np.imag(complex_lorentz_doubleside(freq, f0, A, gamma, phi)) + a_imag * freq + b_imag
    return np.hstack([real_part, imag_part])
